...
def test_get_list_neg():
    # Проверка на запрос при отсутствии токена авторизации
    proj_id = api.get_proj_list_neg()
    assert len(proj_id) > 0
...
def test_change_neg():
    api.change_proj_neg(123, "No more Kadia")
    # Посылаем запрос на поиск по айди,по неправильному айди
